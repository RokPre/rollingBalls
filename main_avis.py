...
# (ball_x [mm], ball_y [mm], ball_vx, ball_vy, rod_positions [0..1], rod_rotations)
...
if __name__ == "__main__":
    ...
    while True:
        ...
        cam1 = state["camData"][1]

        rod_pos = [
            (cam0["rods"][i]["Item1"] + cam1["rods"][i]["Item1"]) / 2.0
            for i in range(8)
        ]
        rod_rot = [
            (cam0["rods"][i]["Item2"] + cam1["rods"][i]["Item2"]) / 2.0
            for i in range(8)
        ]
        ...
        commands_raw = main(sim_state)

        payload = {
            "commands": [
                {
                    "driveID": drive_id,
                    "translationTargetPosition": tran,
                    "rotationTargetPosition": rot,
                    "translationVelocity": tran_vel,
                    "rotationVelocity": rot_vel,
                }
                for drive_id, tran, rot, tran_vel, rot_vel in commands_raw
            ]
        }

        session.post(f"{URL}/Motors/SendCommand", json=payload, timeout=1)
        time.sleep(PERIOD)