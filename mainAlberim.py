...
from defense_modul_v2 import Defense
...
def init():
    ...
    # Defense is delegated to the dedicated defense module. The three rods match
    # drives 1, 2 and 3 in the simulator geometry.
    defense_controller = Defense()
    attack_rod = rod(6)


def defense_command_to_tuple(command: MotorCommand) -> tuple[int, float, float, float, float]:
    """
    Convert a defense-module dictionary command into the legacy tuple format
    expected by run.py / set_motor_command.
    """
    return (
        command["driveID"],
        command["translationTargetPosition"],
        command["rotationTargetPosition"],
        command["translationVelocity"],
        command["rotationVelocity"],
    )
...
def main(sim_state: tuple[float, float, float, float, list[float], list[float]]) -> list[tuple[int, float, float, float, float]]:
    ball_x, ball_y, ball_vx, ball_vy, _, _ = sim_state

    # The simulator exposes velocities in m/s, while the defense module uses the
    # field geometry in millimeters. Convert here so the imported defense logic
    # can stay self-contained.
    defense_commands = defense_controller.step(
        ball_x=ball_x,
        ball_y=ball_y,
        ball_vx=ball_vx * 1000.0,
        ball_vy=ball_vy * 1000.0,
        ball_visible=(0 <= ball_x <= GEOM["field"]["dimension_x"] and 0 <= ball_y <= GEOM["field"]["dimension_y"]),
    )

    attack_command = attack_rod.step(sim_state)

    return [*(defense_command_to_tuple(command) for command in defense_commands), attack_command]
